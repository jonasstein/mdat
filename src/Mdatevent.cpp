...
Mdatevent::Mdatevent()
    : EventID(IDClass::neutron), BufferTimestamp_ns(0b0),
      ...
      EventTrigID(TrigIDClass::CmpReg),
...
Mdatevent::Mdatevent(uint64_t       sortedevent,
    ...
    if (IDClass::trigger == this->EventID) {
        ...
        this->EventData = bitslicer::getintbybitpattern(
            sortedevent,
            0b000000001111111111111111111110000000000000000000);
        ...
        this->EventSlotID = bitslicer::getintbybitpattern(
            sortedevent,
            0b000011111000000000000000000000000000000000000000);
        this->EventAmplitude = bitslicer::getintbybitpattern(
            sortedevent,
            0b000000000111111111100000000000000000000000000000);
        this->EventPosition = bitslicer::getintbybitpattern(
            sortedevent,
            0b000000000000000000011111111110000000000000000000);
    }
    ...
    TimestampClass time19bit_ns =
        ...
        bitslicer::getintbybitpattern(
            sortedevent,
            0b000000000000000000000000000001111111111111111111);
    this->BufferTimestamp_ns = myBufferTimestamp_ns;
    this->FullEventTimestamp_ns =
        myBufferTimestamp_ns + time19bit_ns;
}
...
std::string Mdatevent::printeventverbose(void) {
    ...
    if (this->EventID == IDClass::neutron) {
        ...
        buffer << "FullEventTimestamp: "
               << static_cast<uint64_t>(
                      FullEventTimestamp_ns)
               << " ns\n";

        buffer << "EventModID: "
               << static_cast<uint64_t>(EventModID) << "\n";
        buffer << "EventSlotID: "
    ...
    if (this->EventID == IDClass::trigger) {
        ...
        buffer << "EventDataID: "
               ...
               << "\n";
        buffer << "EventData: "
               << static_cast<uint64_t>(EventData) << "\n";
    }
    return (buffer.str());
}
...
std::string Mdatevent::printevent(void) {
    ...
    if (this->EventID == IDClass::neutron) {
        buffer << static_cast<uint64_t>(
                      FullEventTimestamp_ns)
               << " ";
        buffer << "8 "; // 0..7 are trigger events 8 are
                        // neutron events
