...
: EventID (IDClass::neutron), BufferTimestamp_ns (0b0),
  FullEventTimestamp_ns (0b0), EventModID (0),
  EventSlotID (0), EventAmplitude (0),
  EventPosition (0), EventTrigID (TrigIDClass::CmpReg),
  EventDataID (DataIDClass::ADC1), EventData (0b0)
{
}

Mdatevent::Mdatevent (uint64_t sortedevent, TimestampClass myBufferTimestamp_ns)
{
    ...
    if (IDClass::trigger == this->EventID)
    {
        ...
        this->EventTrigID = getTrigID (
        bitslicer::getintbybitpattern (sortedevent, 0b011100000000000000000000000000000000000000000000));
        this->EventDataID = getDataID (
        bitslicer::getintbybitpattern (sortedevent, 0b000011110000000000000000000000000000000000000000));
        this->EventData =
        bitslicer::getintbybitpattern (sortedevent, 0b000000001111111111111111111110000000000000000000);
    ...
    else
    {
        ...
        bitslicer::getintbybitpattern (sortedevent, 0b011100000000000000000000000000000000000000000000);
        this->EventSlotID =
        bitslicer::getintbybitpattern (sortedevent, 0b000011111000000000000000000000000000000000000000);
        this->EventAmplitude =
        bitslicer::getintbybitpattern (sortedevent, 0b000000000111111111100000000000000000000000000000);
        this->EventPosition =
        bitslicer::getintbybitpattern (sortedevent, 0b000000000000000000011111111110000000000000000000);
    }

    TimestampClass time19bit_ns =
    100 * bitslicer::getintbybitpattern (sortedevent, 0b000000000000000000000000000001111111111111111111);
...
std::string Mdatevent::printeventverbose (void)
{
    ...
    if (this->EventID == IDClass::neutron)
    {
        ...
        buffer << "FullEventTimestamp: "
               << static_cast<uint64_t> (FullEventTimestamp_ns)
               << " ns\n";

        buffer << "EventModID: " << static_cast<uint64_t> (EventModID)
               << "\n";
        buffer << "EventSlotID: "
               << static_cast<uint64_t> (EventSlotID) << "\n";
        buffer << "EventAmplitude: "
               << static_cast<uint64_t> (EventAmplitude) << "\n";
        buffer << "EventPosition: "
               << static_cast<uint64_t> (EventPosition) << "\n";
    }
    ...
    if (this->EventID == IDClass::trigger)
    {
        ...
        buffer << "EventTrigID: "
               << static_cast<uint64_t> (EventTrigID) << "\n";
        buffer << "EventDataID: "
               << static_cast<uint64_t> (EventDataID) << "\n";
